...
def open_file_output():
    ...
    if not file_path:
        return  # No file selected, return without doing anything

    file_text = "文件路径为：" + file_path
    img = Image.open(file_path)

    # Use try-except to catch exceptions that can occur when resizing the image
    try:
        img = image_resize(img)
    except Exception as e:
        print(f"Error while resizing image: {e}")
        return  # Return without displaying the image
